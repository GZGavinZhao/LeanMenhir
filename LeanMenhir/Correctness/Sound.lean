...
inductive WordHasStackSemantics : List G.Token → Stack A → Prop
  | nil : WordHasStackSemantics [] []
  | cons {wordq : List G.Token} {stackq : Stack A} :
      WordHasStackSemantics wordq stackq →
      {wordt : List G.Token} → (s : A.NonInitState) →
      (pt : ParseTree G (A.last_symb_of_non_init_state s) wordt) →
...
(Coq `pop_spec_ptl`). -/
theorem pop_spec_ptl {R : Type} (symbolsToPop : List (Symbol G.Terminal G.Nonterminal))
    (action : arrowsRight R (symbolsToPop.map G.symbol_semantic_type))
    (wordStk : List G.Token) (stk : Stack A) (res : R) (stk' : Stack A)
    (hspec : PopSpec symbolsToPop stk action stk' res)
    (hword : WordHasStackSemantics wordStk stk) :
    ∃ (wordStk' wordRes : List G.Token) (ptl : ParseTreeList G symbolsToPop wordRes),
...
theorem ptSem_cast {s1 s2 : Symbol G.Terminal G.Nonterminal} {w : List G.Token}
    (h : s1 = s2) (pt : ParseTree G s1 w) :
    ptSem (h ▸ pt) = cast (congrArg G.symbol_semantic_type h) (ptSem pt) := by
  subst h; rfl
...
theorem reduce_none_aux (stk : Stack A) (prod : G.Production)
    ...
    (pop (G.prod_rhs_rev prod) stk hpref (G.prod_action prod)).1 = [] ∧
      G.prod_lhs prod = A.start_nt init := by
  ...
  rw [hgoto] at hg
  cases hc : (pop (G.prod_rhs_rev prod) stk hpref (G.prod_action prod)).1 with
  | nil => rw [hc] at hg; exact ⟨rfl, hg⟩
  | cons cell rec => rw [hc] at hg; exact hg.elim

/-- `reduceStep` is sound (Coq `reduce_step_invariant`). -/
theorem reduceStep_sound (stk : Stack A) (prod : G.Production)
    (Hval : validForReduce (stateOfStack init stk) prod) (Hi : StackInvariant init stk)
    (word : List G.Token) (buffer : Buffer G) (hword : WordHasStackSemantics word stk) :
...
theorem step_sound (hsafe : Safe A) (stk : Stack A) (word : List G.Token) (buffer : Buffer G)
    ...
    | .Accept sem bufferNew =>
        ∃ (wordNew : List G.Token) (pt : ParseTree G (.NT (A.start_nt init)) wordNew),
          (word ++ₛ buffer).get = (wordNew ++ₛ bufferNew).get ∧ ptSem pt = sem
    | .Progress stkNew bufferNew =>
        ∃ wordNew : List G.Token,
    ...
    · -- Lookahead
      ...
      · rename_i prod hawt
        have Hv : validForReduce (stateOfStack init stk) prod := by
          ...
          have h2 := h (G.token_term buffer.head); rw [hawt] at h2; exact h2
        have heq2 : reduceStep init stk prod buffer Hv Hi = .Accept sem bn := heqa
        have hr := reduceStep_sound init stk prod Hv Hi word buffer hword
        rw [heq2] at hr
        obtain ⟨pt, hb, hs⟩ := hr
